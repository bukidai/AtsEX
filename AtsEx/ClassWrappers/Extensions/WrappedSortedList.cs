...
{
    public class WrappedSortedList<TKey, TValueWrapper> : IWrappedSortedList<TKey, TValueWrapper>
    {
        protected static readonly Func<TValueWrapper, object> DefaultParserToSource = wrapper => (wrapper as IClassWrapper).Src;
        ...
        {
#if DEBUG
            if (!typeof(TValueWrapper).GetInterfaces().Contains(typeof(IClassWrapper)))
            {
                throw new ArgumentException($"{nameof(TValueWrapper)} が {nameof(IClassWrapper)} を実装しない場合、パラメータ parserToSource を省略することはできません。");
            }
#endif
        }
